  ...
  funct<- function(x,names_x = c("", "", "","","","","","",""),
    ...
    if (is.null(dat.fitting)) { # dat.fitting  = data
      ...
      rss<- sum(sapply(c("Wirte each variable name "), function(nm) { # Add when required variables are present
        ...
        sum((out [, nm ][match(o.time, out [, "time"])] - dat.fitting [dat.fitting$year == nm, "population"])^2, na.rm = TRUE) # When the data shape is changed to melt, the value of value must be expressed.
      }))/(2*nrow(dat.fitting))
    }
    return(MSE)
  }
